...
const BusinessView: React.FC<BusinessViewProps> = ({ sales, contacts, interactions, attempts, isDarkMode = false, dateRange }) => {
  ...
  // Conversion Rate: Sales in Period / New Leads in Period (Cohort approximation)
  const conversionRate = newContactsInPeriod.length > 0 
    ? ((sales.length / newContactsInPeriod.length) * 100).toFixed(1) 
    : (contacts.length > 0 ? ((sales.length / contacts.length) * 100).toFixed(1) : '0');

  const newLeadsCount = newContactsInPeriod.length;

  // --- 2. BI METRICS CALCULATIONS ---

  // A. Sales Cycle (Days from CreatedAt to SaleDate)
  ...
  // C. Weighted Pipeline (Forecast)
  const pipelineValue = contacts.reduce((acc, contact) => {
    if (contact.status === LeadStatus.INTERESTED) return acc + (avgTicket * 0.50);
    if (contact.status === LeadStatus.CONTACTED) return acc + (avgTicket * 0.20);
    if (contact.status === LeadStatus.NEW) return acc + (avgTicket * 0.05);
    return acc;
  }, 0);
