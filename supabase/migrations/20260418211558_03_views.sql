-- ============================================================================
-- Migración 03: Vistas materializadas para KPIs pesados
-- ----------------------------------------------------------------------------
-- Estas MVs reemplazan el O(n×m) de PerformanceView/TeamView y las 15 requests
-- de funnel counts. Incluyen tenant_id explícito; el cliente (anon) debe filtrar
-- por tenant_id. Refresh: CONCURRENTLY en sync jobs. Se crean vacías (WITH NO
-- DATA) porque aún no hay filas en las tablas base.
-- ============================================================================

-- 1) Funnel counts por tenant ------------------------------------------------
...
intentos_agg AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE status = 'Exitoso') AS attempts_successful,
    COUNT(*) FILTER (WHERE status = 'Fallido') AS attempts_failed,
    COUNT(*) FILTER (WHERE status = 'Abandonado') AS attempts_abandoned
...
-- 3) Tendencia mensual de ventas (reemplaza tendencias simuladas - bug #2) ---
DROP MATERIALIZED VIEW IF EXISTS public.mv_sales_trend_monthly CASCADE;
CREATE MATERIALIZED VIEW public.mv_sales_trend_monthly AS
SELECT
  tenant_id,
  date_trunc('month', fecha AT TIME ZONE 'America/Bogota')::date AS mes,
  COUNT(*) AS ventas_cantidad,
  COALESCE(SUM(amount), 0)::numeric(14,2) AS ventas_monto,
  COALESCE(AVG(amount), 0)::numeric(14,2) AS ticket_promedio,
  COUNT(DISTINCT vendedora_id) AS vendedoras_activas
FROM public.ventas
WHERE fecha IS NOT NULL
GROUP BY tenant_id, date_trunc('month', fecha AT TIME ZONE 'America/Bogota')
WITH NO DATA;

CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_sales_trend_unique
  ON public.mv_sales_trend_monthly (tenant_id, mes);

-- 4) Función helper: refresh todas las MVs ----------------------------------
CREATE OR REPLACE FUNCTION public.refresh_materialized_views()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_funnel_counts;
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_vendedora_performance;
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_sales_trend_monthly;
END;
$$;
